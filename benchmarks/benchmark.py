...
x = torch.rand((nsph,), device=device, requires_grad=False)
y = torch.rand((nsph,), device=device, requires_grad=False)
z = torch.rand((nsph,), device=device, requires_grad=False)

torch_time_fw = -time.time()
res = sh(x,y,z)
print(x[0],res[2][:,0])
torch_time_fw += time.time()
print("module time nograd  (fw, bw)  ", torch_time_fw)